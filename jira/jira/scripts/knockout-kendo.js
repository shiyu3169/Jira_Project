...
ko.kendo.BindingFactory = function() {
    ...
    this.setupTemplates = function(templateConfig, options, element, context) {
        var i, j, option, existingDataBoundHandler, existingDetailInitHandler,
        ...
        if (templateConfig && options && options.useKOTemplates) {
            ...
            for (i = 0, j = templateConfig.length; i < j; i++) {
                ...
                if (options[option]) {
                    //setting the detailinit function causes grids without details to be formatted as though they did
                    //check if there is detail in the name of the template being set, if so set a flag to attach
                    //the handler only if a detail template has been added
                    if (/^detail/.test(option)) {
                        containsDetail = true;
                    }
            ...
            existingDataBoundHandler = options.dataBound;
            options.dataBound = function() {
                ko.memoization.unmemoizeDomNodeAndDescendants(element);
                if (existingDataBoundHandler) {
                    existingDataBoundHandler.apply(this, arguments);
                }
            };

            //initialize binding in detailInit event if there is a detail
            if (containsDetail) {
                existingDetailInitHandler = options.detailInit;
                options.detailInit = function (e) {
                    ko.memoization.unmemoizeDomNodeAndDescendants(e.detailRow[0]);
                    if (existingDetailInitHandler) {
                        existingDetailInitHandler.apply(this, arguments);
                    }
                };
            }
    ...
    this.getWidget = function (widgetConfig, options, $element) {
        ...
        if (widgetConfig.parent) {
            ...
            widget = $element[widgetConfig.name](widgetConfig.recursiveUnwrap ? ko.toJS(options) : this.unwrapOneLevel(options)).data(widgetConfig.name);
        }
    ...
    this.watchValues = function (widget, options, widgetConfig, element) {
        ...
        if (watchValues) {
            for (watchProp in watchValues) {
                if (watchValues.hasOwnProperty(watchProp) && Object.prototype.toString.call(watchValues[watchProp]) !== '[object Object]') {
                    self.watchOneValue(watchProp, widget, options, widgetConfig, element);
                } else if (watchValues.hasOwnProperty(watchProp)) {
                    self.watchValuesRecursive(widget, options[watchProp], watchValues[watchProp], element, widgetConfig);
                }
            }
        }
    };

    this.watchValuesRecursive = function (widget, options, watches, element, config) {
        var prop;

        if (watches) {
            for (prop in watches) {
                if (watches.hasOwnProperty(prop) && Object.prototype.toString.call(watches[prop]) !== '[object Object]') {
                    self.watchOneValueFromRecursion(prop, widget, options, config, element, watches);
                } else if (watches.hasOwnProperty(prop)) {
                    self.watchValuesRecursive(widget, options[prop], watches[prop], element, config);
                }
            }
        }
    };

    this.watchOneValueFromRecursion = function (prop, widget, options, widgetConfig, element, watches) {
...
createBinding({
    ...
    templates: ["rowTemplate", "altRowTemplate", "detailTemplate", 'toolbar']
});
...
createBinding({
    ...
    watch: {
        ...
        categoryAxis: {
            labels: {
                format: function (value, options, rootOptions) {
                    var update = { categoryAxis: { labels: { format: value } } };
                    this.setOptions(update);
                }
            },
            field: function (value) {
                var update = { categoryAxis: { field: value } };
                this.setOptions(update);
            }
        },
        series: function (value) {
            this.setOptions({ series: value });
        },
        title: {
            text: function (value, options, rootOptions) {
                this.setOptions({ title: { text: value } });
            }
        }
    }
});
